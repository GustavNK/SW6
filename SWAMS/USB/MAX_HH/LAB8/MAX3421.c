...
// Write N MAX3420E FIFO bytes from the array p
void writebytes(unsigned char reg, unsigned char N, unsigned char *p)
{
	unsigned char j;
	// Slave select (MAX SS low)
	PORTB &= 0b11101111;
	...
	for(j=0; j<N; j++)
	{
		...
		while ((SPSR & 0b10000000) == 0)
		{}
		// dummy store it in the data array
		// = SPDR;
...
void initMAX3421()
{
  ...
  // Modified by Henning Hargaard: The Vbus comparator pin is floating at the USB host shield
  // ----> wreg(rUSBCTL,(bmCONNECT+bmVBGATE));  // VBGATE=1 disconnects D+ pullup if host turns off VBUS
  wreg(rUSBCTL,(bmCONNECT)); // VBGATE=1 disconnects D+ pullup if host turns off VBUS
  EnableIRQs();
